...
- 「特定リソースを誤って更新・削除しない」→ スタックポリシー（更新時のみ）。削除の防止は DeletionPolicy や終了保護（終了保護: 有効にするとスタック削除が失敗する。既定は無効。ネストスタックには親の設定が引き継がれ、ネストスタック単独では変更できない。ただし親スタックの更新でネストスタックが削除される場合は削除される）
